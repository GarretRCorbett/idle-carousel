...
func _ready() -> void:
	...
	# Own shape per enemy, so sizing one never resizes the rest.
	var shape := CircleShape2D.new()
	shape.radius = data.hitbox_radius
	_collision.shape = shape
	_health_bar.max_value = data.base_health
	_health_bar.value = _health
	_health_bar.visible = false
	_health_bar.position = Vector2(
			-_health_bar.size.x / 2.0,
			-data.hitbox_radius - health_bar_gap - _health_bar.size.y)
...
func _flash() -> void:
	...
	_flash_tween.tween_property(_visual, "modulate", Color.WHITE, hit_flash_seconds)
