...
> **Phase 3 Step 9 is BLOCKED on Codex** (out of usage as of 2026-09-26; don't run it until Garret says it's back). When unblocked: the Codex review (`--base b7267ac`) and Garret's sign-off playtest, both in `planning/phase3/step9_plan.md`. Fixes from the review may land on top of Phase 4 work.
> **Phase 4** (`planning/phase4/README.md`): Steps 1–7 done (saves, offline, Auto-Boost, shop + mount levels/stars, upgrade tree, random events) plus the controller foundation. **Next: Step 8, the Park Guide**, planned and handed to a Claude Code cloud session as an experiment (`planning/phase4/cloud_park_guide_prompt.md`); review its PR, then add translations and rebuild the CJK fonts locally. After that: the tutorial, then music. Phase 4 then opens with the shop redesign, levels and stars, auto-boost, the Park Guide, the tutorial and music. The devlog lives in `devlog/` (two post drafts; the voice notes are in `devlog/backstory.md`).
...
- [ ] **Theme / mascot (Garret, 2026-09-26):** exploring a **ticket** mascot (light bulb as backup) and how the theme grows from it: `planning/theme_ticket.md`. Ask Codex for thoughts when its usage is back; then name, premise, and the tutorial's final lines.
